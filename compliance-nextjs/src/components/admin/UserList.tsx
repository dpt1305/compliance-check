...
function computeColWidths(rows: UserListEntry[]): Record<ColKey, number> {
  ...
  for (const row of rows) {
    ...
    check(
      "submitted",
      row.submissionDate ? formatDate(row.submissionDate) : null,
    );
  }
...
export default function UserList() {
  ...
  const [reviewSeedEdit, setReviewSeedEdit] = useState<{
    field: string;
    value: string;
  } | null>(null);
  ...
  useEffect(() => {
    function onMouseMove(e: MouseEvent) {
      ...
      setColWidths((prev) => ({
        ...prev,
        [col]: Math.max(40, startW + e.clientX - startX),
      }));
    }
    function onMouseUp() {
  ...
  const loadPage = useCallback(async (offset: number, reset: boolean) => {
    const { filterProjects, filterMonth, filterYear, filterTags } =
      filterStateRef.current;
    const params = new URLSearchParams();
    params.set("offset", String(offset));
    params.set("limit", String(PAGE_SIZE));
    if (filterProjects !== null)
      filterProjects.forEach((p) => params.append("project", p));
    ...
    try {
      const res = await fetch(`/api/admin/user-list?${params}`, {
        headers: authHeaders(),
      });
  ...
  const sortedItems = useMemo(
    () =>
      [...items].sort((a, b) => compareUserListRows(a, b, sortCol, sortDir)),
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        if (entries[0].isIntersecting) loadPage(currentOffset, false);
      },
      { threshold: 0.1 },
    );
    observer.observe(el);
  ...
  function openReview(row: UserListEntry) {
    const idx = reviewItems.findIndex(
      (r) => r.submissionId === row.submissionId,
    );
  ...
  async function reviewChangeStatus(
    ...
    try {
      ...
      setItems((prev) =>
        prev.map((r) => {
          if (
            r.submissionId !== row.submissionId &&
            r.trackingRowNum !== row.trackingRowNum
          )
            return r;
          ...
          else if (field === "seedConfiguration")
            patch.seedConfiguration = value;
  ...
  function renderCell(
    ...
    const canEdit =
      !isTeamlead &&
      (TRACKING_FIELDS.has(field) || SEED_FIELDS.has(field)
        ? !!row.trackingRowNum
        : false);
    ...
    if (active) {
      return (
        <td
          key={field}
          className={`${className} p-0 align-top border-r border-gray-200`}
          style={{ minWidth: 90 }}
        >
    ...
    return (
      ...
      >
        ...
        {canEdit && (
          <span className="hidden group-hover:inline-block ml-1 text-indigo-300 text-xs">
            ✎
          </span>
        )}
      </td>
  ...
  async function deleteSubmission(entry: UserListEntry) {
    ...
    try {
      const res = await fetch(`/api/admin/submissions/${entry.submissionId}`, {
        method: "DELETE",
        headers: authHeaders(),
      });
      if (!res.ok) throw new Error("Failed");
  ...
  async function deleteMember(entry: UserListEntry) {
    ...
    try {
      const res = await fetch(
        `/api/admin/user-list?rowNum=${entry.trackingRowNum}`,
        {
          method: "DELETE",
          headers: authHeaders(),
        },
  ...
  async function handleUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const body = (await res.json()) as { message: string };
      if (!res.ok) {
        showToast(body.message ?? "Upload failed", false);
        return;
      }
  ...
  async function handleClearPeriod() {
    ...
    try {
      ...
      if (!res.ok) {
        showToast(body.message ?? "Clear failed", false);
        return;
      }
  ...
  return (
    <div className="p-4">
            ...
            <div className="px-5 py-4 grid grid-cols-1 sm:grid-cols-2 gap-3">
              {[
               ...
               <TypeCombobox
                 ...
                 onChange={(v) =>
                   setAddMemberFields((f) => ({ ...f, deviceType: v }))
                 }
                 options={Array.from(
                   new Set(
                     items
                       .map(
                         (row) => row.deviceType ?? row.submissionType ?? "",
                       )
                       .filter(Boolean),
                   ),
                 )}
                  ...
                  <div className="flex items-center gap-2 ml-4 shrink-0">
                    {imgUrl && (
                      <a
                        ...
                        className="btn-icon text-primary-600 text-sm"
        ...
        (() => {
          ...
          try {
            if (row.validationResult)
              aiResult = JSON.parse(row.validationResult as string);
          } catch {
            /* ignore */
          }
          const failedChecks: string[] =
            (aiResult?.failedChecks as string[] | undefined) ?? [];
          const guidelines: string[] =
            (aiResult?.guidelines as string[] | undefined) ?? [];
          const checklist = (aiResult?.checklist ?? {}) as Record<
            string,
            boolean
          >;
          ...
          const aiSuggestion =
            (aiResult?.suggestion as string | undefined) ?? "";
                  ...
                  <div className="flex items-center gap-2 ml-4 shrink-0">
                    {imgUrl && (
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="btn-icon text-primary-600 text-xs"
                        ...
                        {[
                          ...
                          val ? (
                            <div key={label} className="flex gap-1">
                              <dt className="text-gray-400 shrink-0 w-14">
                                {label}
                              </dt>
                    ...
                    <section>
                      ...
                      <dl className="space-y-1">
                        {[
                          ...
                          val ? (
                            <div key={label} className="flex gap-1">
                              <dt className="text-gray-400 shrink-0 w-20">
                                {label}
                              </dt>
                    ...
                    <section>
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        {(
                          [
                            {
                              ...
                              value: row.malwareAlerts,
                            },
                            {
                              field: "complianceChecks",
                              label: "Compliance Checks",
                              value: row.complianceChecks,
                            },
                            {
                              field: "seedConfiguration",
                              label: "SEED Configuration",
                              value: row.seedConfiguration,
                            },
                            {
                          ...
                          const num =
                            display !== null ? parseInt(display, 10) : NaN;
                          ...
                          return (
                            ...
                            >
                              <div className="px-3 pt-2.5 pb-1">
                                <p className="text-gray-500 text-[10px] font-medium leading-tight mb-1">
                                  {label}
                                </p>
                                {isEditing ? (
                                  <div
                                  ...
                                  >
                                    ...
                                    />
                                    {isSeedSaving && (
                                      <span className="spinner w-3 h-3 border-gray-400 border-t-transparent self-center" />
                                    )}
                                  </div>
                                ) : (
                                  <p
                                    className={`text-2xl font-bold leading-none py-1
                              ...
                              {/* Edit pencil hint */}
                              {canEdit && !isEditing && (
                                <span className="absolute top-1.5 right-1.5 text-gray-300 group-hover:text-gray-500 text-[10px]">
                                  ✎
                                </span>
                              )}
                            </div>
                      ...
                      {!row.trackingRowNum && (
                        <p className="mt-1 text-gray-400 italic">
                          No tracking row — values read-only
                        </p>
                      )}
                    </section>
                        ...
                        <ul className="space-y-1">
                          {checklistEntries.map(([key, passed]) => (
                            <li key={key} className="flex items-center gap-1.5">
                              ...
                              <span
                                className={
                                  passed ? "text-gray-600" : "text-red-600"
                                }
                              >
                                {key}
                              </span>
                ...
                <div className="flex items-center justify-between px-5 py-3 border-t border-gray-100 shrink-0 bg-gray-50 rounded-b-xl">
                  ...
                  {!isTeamlead && (
                    <div className="flex items-center gap-2">
                      ...
                      >
                        {isReviewSaving ? (
                          <span className="spinner w-3 h-3 border-yellow-400 border-t-transparent inline-block" />
                        ) : (
                          "⏸ Pending"
                        )}
                      </button>
                      <button
                        disabled={
                          isReviewSaving || currentStatus === "REJECTED"
                        }
                      ...
                      >
                        {isReviewSaving ? (
                          <span className="spinner w-3 h-3 border-red-400 border-t-transparent inline-block" />
                        ) : (
                          "✕ Reject"
                        )}
                      </button>
                      <button
                        disabled={
                          isReviewSaving || currentStatus === "APPROVED"
                        }
                      ...
                      >
                        {isReviewSaving ? (
                          <span className="spinner w-3 h-3 border-white border-t-transparent inline-block" />
                        ) : (
                          "✓ Approve"
                        )}
                      </button>
      ...
      <div className="flex flex-wrap items-center gap-2 mb-2">
        ...
        >
          {filterTags.map((tag) => (
            <span
              key={tag}
              className="flex items-center gap-1 pl-2 pr-1 py-0.5 bg-indigo-100 text-indigo-800 rounded text-xs font-medium whitespace-nowrap"
            >
              {tag}
              <button
                type="button"
      ...
      <div className="flex flex-wrap items-center gap-2 mb-3">
        ...
        {filterMonth && filterYear && !isTeamlead && (
          ...
          >
            {isClearing ? (
              <>
                <span className="spinner w-4 h-4 border-red-400 border-t-transparent"></span>{" "}
                Clearing…
              </>
            ) : (
              <>
        ...
        {!isTeamlead && (
          <button
            onClick={() => {
              if (reviewItems.length > 0) setReviewIdx(0);
            }}
            disabled={reviewItems.length === 0}
            className="btn-secondary flex items-center gap-1.5 text-sm"
            title={
              reviewItems.length > 0
                ? `Review ${reviewItems.length} submission(s)`
            ...
            <tr>
              {(Object.keys(COL_HEADERS) as ColKey[]).map((col) => {
                ...
                return (
                  <th
                    key={col}
                    className="relative select-none whitespace-nowrap overflow-hidden border-r border-gray-200 last:border-r-0"
          ...
          <tbody>
            {sortedItems.length === 0 ? (
              ...
              sortedItems.map((row, idx) => (
                ...
                >
                  {/* No. — not editable */}
                  <td className="text-gray-400 font-medium border-r border-gray-200">
                    {idx + 1}
                  </td>
                  ...
                  {renderCell(
                    row,
                    "account",
                    row.trackingAccount ?? row.account ?? "",
                    "text-gray-500",
                  )}
                  ...
                  {renderCell(
                    row,
                    "deviceType",
                    row.deviceType ?? row.submissionType ?? "",
                    "capitalize",
                  )}
                      ...
                      <select
                        ...
                        onChange={async (e) => {
                          ...
                          setItems((prev) =>
                            prev.map((r) =>
                              r.submissionId === row.submissionId
                                ? { ...r, submissionStatus: newStatus }
                                : r,
                            ),
                          );
                          try {
                            const res = await fetch(
                              `/api/admin/submissions/${row.submissionId}`,
                              {
                                method: "PUT",
                                headers: authHeaders({
                          ...
                          <img
                            ...
                            onError={(e) => {
                              ...
                              el.insertAdjacentHTML(
                                "afterend",
                                '<span class="text-gray-300 text-xs">err</span>',
                              );
                            }}
                          />
                        </button>
      ...
      {hasMore ? (
        <div
          ref={sentinelRef}
          className="flex justify-center items-center py-4 text-sm text-gray-400"
        >
          {isFetchingMore ? (
            <>
