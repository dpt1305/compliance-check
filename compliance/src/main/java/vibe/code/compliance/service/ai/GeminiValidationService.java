...
public class GeminiValidationService implements AiValidationService {
    ...
    public GeminiValidationService(ObjectMapper objectMapper) {
    ...
    public AiValidationResult validate(byte[] imageBytes, String mimeType, String expectedType) {
        ...
        try {
            ...
            if (e.getStatusCode().value() == 429) {
                throw new AiUnavailableException("Gemini rate limit exceeded", e);
            }
            throw new AiUnavailableException("Gemini API error: " + e.getMessage(), e);
        } catch (HttpServerErrorException e) {
            throw new AiUnavailableException("Gemini server error: " + e.getMessage(), e);
        } catch (Exception e) {
            throw new AiUnavailableException("Gemini AI validation failed: " + e.getMessage(), e);
        }
    }
