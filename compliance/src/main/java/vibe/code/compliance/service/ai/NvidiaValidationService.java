...
@Service
@Slf4j
public class NvidiaValidationService implements AiValidationService {
    ...
    @Value("${ai.nvidia.endpoint:}")
    private String endpoint;

    @Value("${ai.nvidia.model:}")
    private String model;

    private final RestClient restClient;
    private final ObjectMapper objectMapper;

    public NvidiaValidationService(ObjectMapper objectMapper) {
    ...
    public AiValidationResult validate(byte[] imageBytes, String mimeType, String expectedType) {
        if (apiKey == null || apiKey.isBlank()) {
            throw new AiUnavailableException("NVIDIA API key not configured");
        }
        if (endpoint == null || endpoint.isBlank()) {
            throw new AiUnavailableException("NVIDIA endpoint not configured");
        }
        if (model == null || model.isBlank()) {
            throw new AiUnavailableException("NVIDIA model not configured");
        }
        ...
        try {
            ...
            Map<String, Object> body = Map.of(
                    "model", model,
                    "messages", List.of(
                            Map.of("role", "system", "content", selectPrompt(expectedType)),
            ...
            String fullEndpoint = endpoint.endsWith("/v1/chat/completions") ? endpoint : endpoint + "/v1/chat/completions";
            ...
            if (e.getStatusCode().value() == 429) {
                throw new AiUnavailableException("NVIDIA rate limit exceeded", e);
            }
            throw new AiUnavailableException("NVIDIA API error: " + e.getMessage(), e);
        } catch (HttpServerErrorException e) {
            throw new AiUnavailableException("NVIDIA server error: " + e.getMessage(), e);
        } catch (Exception e) {
            throw new AiUnavailableException("NVIDIA AI validation failed: " + e.getMessage(), e);
        }
    }
